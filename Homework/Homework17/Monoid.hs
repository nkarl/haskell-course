...
instance Monoid Config where
  mempty = Config{port = 4000, host = "example.com"}
...
instance Semigroup Emergency where
  ...
  (<>) s1 s2 =
    Emergency
      ...
      , description = description s1 <> description s2
      , resolved = resolved s1 <> resolved s2
      }

instance Monoid Emergency where
  mempty = Safe -- Emergency{severity = Low, description = "No risk", resolved = All True}
